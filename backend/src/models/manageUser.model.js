...
class ManageUserModel {
    ...
    create = async ({ email, have_wallet, wallet_address, name }) => {
        try {
            const sql = `INSERT INTO ${this.tableName}
            (email, have_wallet, wallet_address, name) VALUES (?,?,?,?)`;

            const result = await query(sql, [email, have_wallet, wallet_address, name]);
